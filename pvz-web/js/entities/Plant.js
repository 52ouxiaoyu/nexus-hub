class Plant extends Entity {
    constructor(game, type) {
        ...
        const getStats = (t) => {
            ...
            stat.fireRate = 1.0;
            stat.fireTimer = 0;
            stat.src = 'assets/images/Plants/MelonPult/MelonPult.png?v=1790424705';
        } else if (type === 'wintermelon') {
            stat.hp = 300;
            stat.fireRate = 1.0;
            stat.fireTimer = 0;
            stat.src = 'assets/images/Plants/WinterMelon/WinterMelon.png?v=1790424705';
            ...
            stat.fireRate = 1.4;
            stat.fireTimer = 0;
            stat.src = 'assets/images/Plants/CabbagePult/CabbagePult.png?v=1790424705';
            ...
            stat.fireRate = 1.4;
            stat.fireTimer = 0;
            stat.src = 'assets/images/Plants/KernelPult/KernelPult.png?v=1790424705';
            ...
            stat.hp = 600;
            stat.src = 'assets/images/Plants/CobCannon/CobCannon.png?v=1790424705';
            ...
            stat.hp = 300;
            stat.src = 'assets/images/Plants/Plantern/0.gif?v=1790424705';
            stat.yOffset = 0;
        }
    ...
    triggerBombFusion() {
        ...
        if (!this.game.fusionMode && !this.game.vaseMode) return;
                ...
                if (targetPlant && !targetPlant.isDead && targetPlant !== this) {
                    try {
                        ...
                        if (fusionResult) {
                            ...
                            if (this.game._inheritMineState) this.game._inheritMineState(newPlant, targetPlant); // v3.34.0 土豆雷武装状态继承
    ...
    explodeNow() {
        ...
        if (this.hasTrait('cherrybomb') || this.hasTrait('jalapeno')) {
            ...
            for (let z of zombies) {
                if (this.hasTrait('cherrybomb') && this.hasTrait('snowpea')) {
                    if (Math.abs(z.row - this.row) <= 1 && Math.abs(z.x - this.x) < 100) {
                        z.takeDamage(900, { bomb: true, oneshot: true }); // half damage
                        z.setSlow(10.0);
                    }
            ...
            if (this.hasTrait('cherrybomb')) {
                // v3.20.0：不再把本体元素放大 1.5 倍充作爆炸（"变大一半就直接消失"）——
                // 改为在场地上叠一张覆盖 3×3 格的原版 Boom.gif，播放完整爆破动画
    ...
    _showCobShell(show) {
        if (show) {
            if (!this._cobShellEl) {
                ...
                el.src = 'assets/images/Plants/CobCannon/CobLoaded.png?v=1790424705';
                ...
                if (this.type === 'fusion_doomshroom_sunflower') {
                    ...
                    for (let zombie of zombies) {
                        ...
                        if (Math.abs(zombie.row - this.row) <= 2 && Math.abs(zombie.x - this.x) < 200) {
                            zombie.takeDamage(1800, { bomb: true, oneshot: true });
                        }
                    }
                } else {
                    const zombies = this.game.entities.filter(e => e instanceof Zombie && !e.isDead && e.state !== 'DYING');
        ...
        if (this.type === 'fusion_starfruit' || this.type === 'starfruit') {
            ...
            if (this.fireTimer >= this.fireRate) {
                ...
                const hasEnemy = this.game.entities.some(e =>
                    e instanceof Zombie && !e.isDead && e.state !== 'DYING' && !e.hypnotized &&
                    // v3.20.0：五星覆盖前后 —— 前方任意方向 + 同行身后 4 格内有敌人也齐射
                    (e.x > this.x - 20 || (e.row === this.row && e.x < this.x && this.x - e.x < 320))
                );
                if (hasEnemy) {
                    this.fireTimer = 0;
...
let isHybridSun = this.hasTrait('peashooter') || this.hasTrait('snowpea') || this.hasTrait('repeater');
            ...
            if (!this.hasExploded) {
                // v3.33.0：被手套拎起（拖拽融合途中）时引信暂停 —— 否则还没拖到目标豌豆就在半路炸了
                ...
                if (this.hasTrait('cherrybomb')) {
                    // v3.20.0：同 explodeNow —— 叠 3×3 原版 Boom.gif，不再本体放大充数
                    this.element.style.visibility = 'hidden';
                    const boom = document.createElement('img');
                    boom.src = 'assets/images/Plants/CherryBomb/Boom.gif';
                    boom.style.cssText = 'position:absolute;pointer-events:none;z-index:3000;' +
        ...
        if (this.hasTrait('spikeweed') || this.hasTrait('spikerock')) {
            ...
            if (this.damageTimer >= 0.75) { // v3.14.0：攻速=2×豌豆射手(1.5s/发)
                ...
                if (zombies.length > 0) {
                    // v3.25.0：地刺专属"扎刺"音（旧版借用保龄球撞击声，无个人特色）
        ...
        if (this.hasTrait('gloomshroom')) {
            ...
            if (this.fireTimer >= 1.0) {
                ...
                if (zombies.length > 0) {
                    this.fireTimer = 0;
                    // v3.25.0：忧郁菇专属"孢子云"音（旧版借用保龄球撞击声）
                    if (this.game.audioManager.playFx) this.game.audioManager.playFx('gloom_burst');
                    else this.game.audioManager.play('splat');
            ...
            if (!this.isArmed) {
                ...
                if (zombieNear) {
                    ...
                    this.element.style.zIndex = 10;
                    if (this.type === 'fusion_sporemine') {
                        // 孢子地雷（小喷菇+土豆地雷）：不再"一个地雷换一只普通僵尸"——
                        // 蘑菇部分让它在炸完后 1.5 秒重新聚拢孢子、15 秒后再武装，可反复使用
                        setTimeout(() => {
                            if (this.isDead || this.hp <= 0) return;
